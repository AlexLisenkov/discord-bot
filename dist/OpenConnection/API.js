...
function default_1() {
    ...
    app.use((req, res, next) => {
        ...
        axios_1.default.get(apiUrl + '/users/@me', headers).then((discordResponse) => {
    ...
    app.get('/guilds', (req, res) => {
        ...
        axios_1.default.get(apiUrl + '/users/@me/guilds', headers).then((discordResponse) => {
            const knownGuilds = [];
            for (let x in discordResponse.data) {
                const guild = discordResponse.data[x];
                if (VoiceConnections_1.default.getGuilds()[guild['id']]) {
                    guild.admin = VoiceConnections_1.default.getGuilds()[guild['id']].guild.members.find('id', res.locals.user.id).hasPermission('ADMINISTRATOR');
    ...
    app.get('/guilds/:guildId', (req, res) => {
        const guild = VoiceConnections_1.default.getGuilds()[req.params.guildId];
        if (!guild) {
            res.status(404);
    ...
    app.get('/guilds/:guildId/commands', (req, res) => {
        ...
        SetDJCommandCommand_1.default.availableCommands.forEach(command => {
    ...
    app.get('/guilds/:guildId/disallowedVoiceChannels', (req, res) => {
        ...
        guild.disallowedVoiceChannels.forEach(voiceChannelId => {
            const channel = guild.guild.channels.find('id', voiceChannelId);
            response.push({
                id: channel.id,
                name: channel.name,
                type: channel.type,
